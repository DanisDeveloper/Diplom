        ...
        <div class="btns">
          ...
          <div class="right-btns">
            ...
            <button v-else v-if="this.$store.state.isAuth" :class="{'btn-border-error': titleEmpty}" class="action-icon-btn" @click="handleSaveOrForkButtonClick">
              <save-icon v-tooltip="'Save shader'" :color="titleEmpty? '#f44336' : 'lightgrey'" v-if="isStoreUser || this.id === null"/>
              <fork-icon v-tooltip="'Fork shader'" v-else/>
            </button>
          </div>
        </div>
      </div>
      ...
      <div v-if="this.id || this.comments.length > 0" class="comments-area">
        ...
        <div v-for="comment in comments" :key="comment['id']" class="comment">
          ...
          <div v-else v-if="this.$store.state.user.id === comment['user_id']" class="comment__hide">
            <unhide-icon v-tooltip="'Unhide comment'" :width="24" :height="24" v-if="comment['hidden']" @click="handleHideButton(comment)"></unhide-icon>
            <hide-icon v-tooltip="'Hide comment'" :width="24" :height="24" v-else @click="handleHideButton(comment)"></hide-icon>
          </div>
